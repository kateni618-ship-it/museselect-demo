...
- Added local keyword search to each PICKS tab, added AI LOOKS status filtering for All, Ready, and Generating, and simplified MINE by moving `View Earnings` into the earnings summary while placing Analytics in the same menu group as My Design and My Products.
...
- Updated Earning / View Earnings: renamed Confirmed to Settled, removed order source channels from commission rows, added default 90d time filtering with Total, moved settlement/type filters behind a right-aligned filter icon, shortened AI Look Boost labels, removed order status tags from Bonus rows, and showed Closed rows as $0 with the original amount struck through underneath.
