  ...
  return pageShell(
    `
      <section class="product-detail-hero">
        ...
        <section class="tryon-guide-module" aria-label="No sample needed">
          <span>
            <b>No sample needed</b>
            <strong>Create AI Tryon Looks for your vibe.</strong>
          </span>
          <ul>
            <li>Skip samples.</li>
            <li>Post faster.</li>
      ...
      <section class="product-info">
        ...
        <div class="detail-commission-panel">
          ...
          <button class="ai-commission" type="button" data-create-look-product="${product.id}">
            <span class="ai-content-tag">${icons.sparkle}<i>AI Tryon Looks</i></span>
      ...
      <button class="tryon-floating-button" type="button" data-create-look-product="${product.id}">TRYON LOOKS</button>
    `,
    "for-u"
  );
}
...
export function bindProductDetailInteractions(root) {
  ...
  root.querySelectorAll("[data-create-look-product]").forEach((button) => {
    button.addEventListener("click", () => {
      navigate(`/create-look?product=${button.dataset.createLookProduct}`);
    });
  });
...
function showPromotionSheet(product, collection) {
  ...
  document.querySelector("[data-promo-nav]")?.addEventListener("click", (event) => {
    ...
    document.querySelectorAll("[data-promotion-sheet]").forEach((node) => node.remove());
    navigate(collection ? `/create-look?collection=${collection.id}` : `/create-look?product=${product.id}`);
  });
}
